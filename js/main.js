...
const boxMaterial = new THREE.MeshStandardMaterial({color: 0xFFFFFF, metalness: 0.5, roughness: 56, }); // Define material // Simple white box
const boxMesh = new THREE.Mesh(boxGeometry, boxMaterial); // Build box
boxMesh.rotation.set(10, 0, 10); // Set box initial rotation
scene.add(boxMesh); // Add box to canvas

// Create spheres: 
const sphereMeshes = [];
const sphereGeometry = new THREE.SphereGeometry(0.181, 32, 32); // Define geometry
...
const pointLight = new THREE.PointLight({colour: 'white', intensity: 8, dist: 12, x: 1, y: 3, z: 0})
